...
int main(int argc, char const *argv[]){
    ...
    if (argc != 4){
        ...
        int cmp_jpeg = strcmp(input_file_extension , JPEG_EXTENSION);
        ...
        if(cmp_png == 0){
            ...
            // lightning_intensity_file = fopen(input_file , "w+");
            // if (lightning_intensity_file == NULL) {
            //     abort_("[write_lightning_intensity] Error during opening file");
            // }

            printf("Computing spatial frequences ...");
            float** lightning_intensity = compute_lightning_frequences(image_data , NULL , width , height);
            printf("SUCESS\n\n");

            // write_lightning_intensity(lightning_intensity , lightning_intensity_file , input_file);
            ...
            for (int y = 0 ; y < height ; y++){
                for (int x = 0 ; x < width ; x++){
                    ...
                    image_data[offset] = lightning_intensity[x][y];  
                    image_data[offset + 1] = lightning_intensity[x][y];  
                    image_data[offset + 2] = lightning_intensity[x][y]; 
                }
            }
